...
router.get('/sorry', ensureAuthenticated, function (req, res) {
  res.render('sorry');
})
...
//admin memberlist details show code here.......

router.get('/memberdetails', function (req, res, next) {

  res.render('memberdetails');
});

//Api for get item list
router.get('/GetMemberdetails', ensureAuthenticated, (req, res) => {
  var groupid = req.user.groupid;
  conditionQueryGroupid = { groupid: { $ne: groupid } }
  User.find(conditionQueryGroupid, (err, content) => {
    res.send(content);
  });
});
...
//   var userid = req.user._id;
//   conditionQueryUserid = { userid: userid }
//   Group.find(conditionQueryUserid, (err, content) => {
//     res.send(content);
//   });
// });
